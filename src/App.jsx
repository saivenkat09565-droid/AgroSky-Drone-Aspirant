...
const STATIC_PRODUCTS = [
  { id:1,  name:"AgroSky Agri Drone 10L",              price:750000,  originalPrice:900000,  image:"https://framerusercontent.com/images/J2SsjH2XcUHn6jAVX44tSmKJ8.png",              isNew:true,  isOffer:true,  status:"instock",    category:"Drones",           waNum:"919390238537" },
  { id:2,  name:"AgroSky Flash Q20 TC Drone",           price:850000,  originalPrice:1000000, image:"https://framerusercontent.com/images/eRu3lhJevMrkK1YctwXEMpgZICM.png",           isNew:false, isOffer:true,  status:"instock",    category:"Drones",           waNum:"919390238537" },
  { id:3,  name:"14S 22000mAh AgroSky VOLT Plus",       price:40000,   originalPrice:55000,   image:"https://framerusercontent.com/images/oDDN2aWnPwDBOacMrdJxrPj3K8.png",            isNew:false, isOffer:true,  status:"instock",    category:"Batteries",        waNum:"919390238537" },
  { id:4,  name:"14S 30000mAh AgroSky VOLT Plus",       price:50000,   originalPrice:65000,   image:"https://framerusercontent.com/images/EIRN6BISlMewm9CBHoIWVhtzYak.png",           isNew:true,  isOffer:true,  status:"instock",    category:"Batteries",        waNum:"919390238537" },
...
const DEFAULT_BANNERS = [
  {
    ...
    badge: "LIMITED TIME",
    bg: "linear-gradient(135deg,#7f1d1d 0%,#dc2626 60%,#7f1d1d 100%)",
    emoji: "🚁",
    cta: "Shop Now",
  },
  {
    id: 2,
    title: "NEW BATTERIES",
    subtitle: "AgroSky VOLT Plus 14S Series – In Stock",
    badge: "JUST ARRIVED",
    bg: "linear-gradient(135deg,#1f2937 0%,#374151 60%,#1f2937 100%)",
    emoji: "🔋",
    cta: "View Range",
  },
  {
    ...
    badge: "HOT DEAL",
    bg: "linear-gradient(135deg,#450a0a 0%,#991b1b 60%,#450a0a 100%)",
    emoji: "🕹️",
    cta: "Grab Now",
  },
];
...
function useToast() {
  ...
  const el = (
    <div style={{
      ...
      border: `1px solid ${toast.type === "success" ? "#dc2626" : "#e05050"}`,
...
function AuthModal({ onClose, onLogin }) {
  ...
  return (
    <div style={{
      ...
      <div style={{
        background:"#ffffff",border:"1px solid rgba(220,38,38,0.25)",borderRadius:20,
        padding:"36px 28px",width:"100%",maxWidth:400,position:"relative",
      }}>
        <button onClick={onClose} style={{
          position:"absolute",top:14,right:16,background:"none",border:"none",
          color:"#9ca3af",fontSize:"1.3rem",cursor:"pointer"
        }}>✕</button>
        <img src={LOGO} alt="AgroSky" style={{ height:40, marginBottom:14, display:"block", margin:"0 auto 14px" }} />
        <h2 style={{ fontFamily:"'Barlow Condensed',sans-serif",fontSize:"1.7rem",fontWeight:800,color:"#111827",textAlign:"center",marginBottom:4 }}>
          {tab === "login" ? "Welcome Back" : "Create Account"}
        </h2>
        <p style={{ fontSize:"0.83rem",color:"#6b7280",textAlign:"center",marginBottom:24 }}>
          {tab === "login" ? "Sign in to your AgroSky Drone Aspirant account" : "Join AgroSky Drone Aspirant"}
        </p>
        <div style={{ display:"flex",background:"#f3f4f6",borderRadius:40,padding:3,marginBottom:22 }}>
          {["login","register"].map(t => (
            <button key={t} onClick={() => switchTab(t)} style={{
              ...
              color: tab===t ? "#f5f5f5" : "#6b7280",transition:"all .2s"
            }}>{t === "login" ? "Sign In" : "Register"}</button>
          ))}
        </div>
        {tab === "register" && (
          <div style={{ marginBottom:12 }}>
            <label style={{ fontSize:"0.71rem",color:"#6b7280",fontWeight:700,letterSpacing:".08em",textTransform:"uppercase",display:"block",marginBottom:4 }}>Full Name</label>
            <input placeholder="Ravi Kumar" value={name} onChange={e => setName(e.target.value)}
              style={{ width:"100%",padding:"10px 13px",background:"#ffffff",border:"1.5px solid rgba(220,38,38,0.2)",borderRadius:10,color:"#111827",fontFamily:"'DM Sans',sans-serif",fontSize:"0.9rem",outline:"none",boxSizing:"border-box" }} />
          </div>
        )}
        <div style={{ marginBottom:12 }}>
          <label style={{ fontSize:"0.71rem",color:"#6b7280",fontWeight:700,letterSpacing:".08em",textTransform:"uppercase",display:"block",marginBottom:4 }}>Email</label>
          <input type="email" placeholder="you@example.com" value={email} onChange={e => setEmail(e.target.value)}
            onKeyDown={e => e.key === "Enter" && (tab==="login"?doLogin():doRegister())}
            style={{ width:"100%",padding:"10px 13px",background:"#ffffff",border:"1.5px solid rgba(220,38,38,0.2)",borderRadius:10,color:"#111827",fontFamily:"'DM Sans',sans-serif",fontSize:"0.9rem",outline:"none",boxSizing:"border-box" }} />
        </div>
        <div style={{ marginBottom:16 }}>
          <label style={{ fontSize:"0.71rem",color:"#6b7280",fontWeight:700,letterSpacing:".08em",textTransform:"uppercase",display:"block",marginBottom:4 }}>Password</label>
          <input type="password" placeholder="••••••••" value={password} onChange={e => setPassword(e.target.value)}
            onKeyDown={e => e.key === "Enter" && (tab==="login"?doLogin():doRegister())}
            style={{ width:"100%",padding:"10px 13px",background:"#ffffff",border:"1.5px solid rgba(220,38,38,0.2)",borderRadius:10,color:"#111827",fontFamily:"'DM Sans',sans-serif",fontSize:"0.9rem",outline:"none",boxSizing:"border-box" }} />
        </div>
        <button onClick={tab==="login"?doLogin:doRegister} disabled={loading} style={{
          width:"100%",padding:13,background:"#dc2626",color:"#f5f5f5",border:"none",borderRadius:40,
        ...
        {info && <div style={{ marginTop:12,color:"#dc2626",fontSize:"0.82rem",background:"rgba(220,38,38,0.1)",borderRadius:8,padding:"8px 12px",border:"1px solid rgba(220,38,38,0.3)" }}>{info}</div>}
      </div>
    </div>
  );
}
...
function CartDrawer({ open, onClose, cart, user, showAuth, updateCartQty, removeFromCart }) {
  ...
  const checkout = async () => {
    ...
    const msg = `🛒 *Cart Enquiry — AgroSky Drone Aspirant*\n\n👤 *Customer:* ${user.name}\n✉️ *Email:* ${user.email||''}\n\n*Items:*\n${lines}\n\n💰 *Total: ${formatINR(total)}*\n\nPlease confirm availability. Thank you!`;
    if (user.accessToken) {
  ...
  return (
    <>
      ...
      <div style={{
        ...
        background:"#ffffff",borderLeft:"1px solid rgba(220,38,38,0.2)",
        display:"flex",flexDirection:"column",
        transform: open ? "translateX(0)" : "translateX(100%)",
        transition:"transform .35s cubic-bezier(.4,0,.2,1)"
      }}>
        <div style={{ display:"flex",alignItems:"center",justifyContent:"space-between",padding:"18px 20px",borderBottom:"1px solid rgba(220,38,38,0.13)" }}>
          <span style={{ fontFamily:"'Barlow Condensed',sans-serif",fontSize:"1.25rem",fontWeight:800,color:"#111827" }}>
            🛒 Cart {cart.length > 0 && <span style={{ color:"#dc2626" }}>({cart.length})</span>}
          </span>
          <button onClick={onClose} style={{ background:"none",border:"none",color:"#6b7280",fontSize:"1.3rem",cursor:"pointer" }}>✕</button>
        </div>
        <div style={{ flex:1,overflowY:"auto",padding:16 }}>
          {cart.length === 0 ? (
            <div style={{ display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",height:"100%",color:"#6b7280",gap:10 }}>
              <div style={{ fontSize:"3rem",opacity:.4 }}>🛒</div>
              <div style={{ fontSize:"0.9rem" }}>Your cart is empty</div>
            </div>
          ) : cart.map(item => (
            <div key={item.id} style={{ display:"flex",gap:12,padding:"12px 0",borderBottom:"1px solid rgba(220,38,38,0.1)" }}>
              ...
              <div style={{ flex:1 }}>
                ...
                <div style={{ fontFamily:"'Barlow Condensed',sans-serif",fontSize:"1rem",fontWeight:800,color:"#dc2626" }}>{formatINR(item.price*item.qty)}</div>
                <div style={{ display:"flex",alignItems:"center",gap:8,marginTop:6 }}>
                  <button onClick={() => updateCartQty(item.id, item.qty - 1)} style={{ width:24,height:24,borderRadius:"50%",border:"1px solid rgba(220,38,38,0.3)",background:"#f5f5f5",color:"#1f2937",cursor:"pointer",fontSize:"0.85rem",display:"flex",alignItems:"center",justifyContent:"center" }}>−</button>
                  <span style={{ fontSize:"0.88rem",fontWeight:600,color:"#111827",minWidth:18,textAlign:"center" }}>{item.qty}</span>
                  <button onClick={() => updateCartQty(item.id, item.qty + 1)} style={{ width:24,height:24,borderRadius:"50%",border:"1px solid rgba(220,38,38,0.3)",background:"#f5f5f5",color:"#1f2937",cursor:"pointer",fontSize:"0.85rem",display:"flex",alignItems:"center",justifyContent:"center" }}>+</button>
                </div>
              </div>
              <button onClick={() => removeFromCart(item.id)} style={{ background:"none",border:"none",color:"#6b7280",cursor:"pointer",fontSize:"1rem",alignSelf:"flex-start" }}>✕</button>
            </div>
          ))}
        </div>
        {cart.length > 0 && (
          <div style={{ padding:"14px 18px",borderTop:"1px solid rgba(220,38,38,0.13)" }}>
            <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14 }}>
              <span style={{ fontSize:"0.88rem",color:"#6b7280" }}>Total</span>
              <span style={{ fontFamily:"'Barlow Condensed',sans-serif",fontSize:"1.35rem",fontWeight:800,color:"#111827" }}>{formatINR(total)}</span>
            </div>
            <button onClick={checkout} style={{
              width:"100%",padding:13,background:"#dc2626",color:"#f5f5f5",border:"none",
...
function ProductDetailModal({ product, onClose, onAddCart, user, showAuth, allProducts, onSimilarClick }) {
  ...
  const msg = `Hello AgroSky Drone Aspirant! 👋\n\nI'm interested in: ${product.name}\nPrice: ${formatINR(product.price)}\n\nPlease share more details.`;
  ...
  // Product highlights
  const highlights = [
    { label:"Brand",         value:"AgroSky Drone Aspirant" },
  ...
  return (
    <div style={S.page}>
      ...
      <div style={{ background:"#fff", padding:"16px 16px 20px", marginBottom:8, borderBottom:"1px solid #e5e7eb" }}>
        <div style={{ fontSize:"0.67rem", fontWeight:700, color:"#fca5a5",
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
          <button onClick={handleAddCart} style={{
            background:"#fca5a5", color:"#0c1b2e", border:"none", borderRadius:40,
          ...
          <a href={waLink(product.waNum||"919390238537", msg)} target="_blank" rel="noreferrer" style={{
            border:"1.5px solid #fca5a5", background:"#fff", color:"#0369a1",
      ...
      <div style={S.card}>
        ...
        <p style={{ fontSize:"0.85rem", color:"#4b5563", lineHeight:1.8, margin:0 }}>
          ...
          and AgroSky Drone Aspirant' trusted after-sale support. Designed for professional
        ...
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:14 }}>
          ...
          {user && !alreadyReviewed && !showForm && (
            <button onClick={()=>setShowForm(true)} style={{
              background:"rgba(133,201,255,0.12)", border:"1px solid #fca5a5",
          ...
          <div style={{ background:"#f0f9ff", border:"1px solid #bae6fd",
            ...
            <div style={{ display:"flex", gap:8, marginTop:10 }}>
              <button onClick={submitReview} disabled={submitting} style={{
                flex:1, background:"#fca5a5", color:"#0c1b2e", border:"none",
                    ...
                    <div key={star} style={{ display:"flex", alignItems:"center",
                      ...
                      <div style={{ flex:1, height:6, background:"#e5e7eb",
                        ...
                        <div style={{ width:`${pct}%`, height:"100%",
                          background:"#fca5a5", borderRadius:4, transition:"width .3s" }} />
                      </div>
                ...
                <div style={{ display:"flex", justifyContent:"space-between",
                  ...
                  <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                    <div style={{ width:32, height:32, borderRadius:"50%",
                      background:"linear-gradient(135deg,#fca5a5,#38bdf8)",
                      display:"flex", alignItems:"center", justifyContent:"center",
...
function BannerCarousel({ banners, onCategoryLink }) {
  ...
  return (
    <div style={{ margin:"12px 14px 0",borderRadius:16,overflow:"hidden",position:"relative",boxShadow:"0 4px 20px rgba(220,38,38,0.15)" }}>
      <div style={{
        minHeight:190,position:"relative",display:"flex",flexDirection:"column",justifyContent:"center",
        background: b.imageUrl ? "transparent" : (b.bg || "linear-gradient(135deg,#7f1d1d 0%,#dc2626 100%)"),
        overflow:"hidden"
      }}>
        ...
        <div style={{ position:"relative",zIndex:2,padding:"22px 20px" }}>
          ...
          {(b.cta || b.linkCategory) && (
            <button
              ...
              style={{
                alignSelf:"flex-start",background:"#fff",color:"#7f1d1d",border:"none",borderRadius:40,
      ...
      <div style={{ display:"flex",justifyContent:"center",gap:6,padding:"8px 0",background:"rgba(0,0,0,0.08)" }}>
        {banners.map((_,i) => (
          <div key={i} onClick={() => goTo(i)} style={{
            ...
            background: i===idx ? "#dc2626" : "rgba(220,38,38,0.3)",
...
function BannerAdminModal({ banners, onSave, onClose }) {
  ...
  const [form, setForm] = useState({ title:"", subtitle:"", badge:"", bg:"linear-gradient(135deg,#7f1d1d 0%,#dc2626 100%)", emoji:"🚁", cta:"Shop Now", imageUrl:"", linkCategory:"" });
  ...
  const openEdit = (i) => {
    setEditIdx(i);
    setForm(i === -1
      ? { title:"", subtitle:"", badge:"", bg:"linear-gradient(135deg,#7f1d1d 0%,#dc2626 100%)", emoji:"🚁", cta:"Shop Now", imageUrl:"", linkCategory:"" }
            ...
            <div>
              ...
              <div style={{ marginBottom:12 }}>
                ...
                <div
                  ...
                  style={{
                    width:"100%",height:100,border:"2px dashed #fca5a5",borderRadius:12,
                  ...
                  {form.imageUrl
                    ...
                    : <><div style={{ fontSize:"2rem",marginBottom:4 }}>📷</div><div style={{ fontSize:"0.78rem",color:"#fca5a5",fontWeight:600 }}>Click to upload image</div></>
                  }
                </div>
              ...
              <div style={{ marginBottom:14 }}>
                ...
                {form.linkCategory && <div style={{ marginTop:5,fontSize:"0.74rem",color:"#dc2626" }}>Button will navigate to: <strong>{form.linkCategory}</strong></div>}
              </div>
              ...
              {!form.imageUrl && (
                <div style={{ marginBottom:14 }}>
                  ...
                  <div style={{ marginTop:8 }}>
                    ...
                    <input value={form.bg} onChange={e => setForm(f => ({...f,bg:e.target.value}))} placeholder="linear-gradient(135deg,#7f1d1d 0%,#dc2626 100%)"
              ...
              <div style={{ display:"flex",gap:10 }}>
                <button onClick={saveForm} style={{ flex:1,padding:"11px",background:"#dc2626",color:"#fff",border:"none",borderRadius:40,fontFamily:"'DM Sans',sans-serif",fontWeight:700,cursor:"pointer" }}>Save Banner</button>
            ...
            <>
              {list.map((b,i) => (
                <div key={b.id} style={{ display:"flex",alignItems:"center",gap:12,padding:"10px 0",borderBottom:"1px solid #f3f4f6" }}>
                  ...
                  <button onClick={()=>openEdit(i)} style={{ background:"#fff1f1",border:"1px solid #fca5a5",color:"#dc2626",borderRadius:8,padding:"5px 11px",fontSize:"0.76rem",cursor:"pointer",fontFamily:"'DM Sans',sans-serif",fontWeight:700 }}>Edit</button>
              ...
              <button onClick={()=>openEdit(-1)} style={{ width:"100%",marginTop:14,padding:"11px",background:"#fff1f1",color:"#dc2626",border:"1.5px dashed #fca5a5",borderRadius:12,fontFamily:"'DM Sans',sans-serif",fontWeight:700,fontSize:"0.88rem",cursor:"pointer" }}>
                + Add New Banner
              </button>
              {saveError && (
              ...
              <button onClick={saveAll} disabled={saving} style={{ width:"100%",marginTop:10,padding:"11px",background: saving ? "#9ca3af" : "#dc2626",color:"#fff",border:"none",borderRadius:40,fontFamily:"'DM Sans',sans-serif",fontWeight:700,fontSize:"0.9rem",cursor:saving?"wait":"pointer",opacity:saving?0.8:1 }}>
    ...
    <div style={{ background:"#f5f7fa",minHeight:"100vh",paddingBottom:70,fontFamily:"'DM Sans',sans-serif" }}>
      ...
      <div style={{
        ...
        <div style={{ display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:8 }}>
          <div style={{ display:"flex",alignItems:"center",gap:8 }}>
            <img src={LOGO} alt="AgroSky" style={{ height:34,borderRadius:6 }} />
            <div>
              <div style={{ fontFamily:"'Barlow Condensed',sans-serif",fontWeight:800,fontSize:"1.05rem",color:"#fff",lineHeight:1,letterSpacing:"-0.01em" }}>AGROSKY DRONE ASPIRANT</div>
            </div>
          </div>
          <div style={{ display:"flex",alignItems:"center",gap:8 }}>
            ...
            {localUser ? (
              <div style={{
                width:34,height:34,borderRadius:"50%",background:"#dc2626",
                display:"flex",alignItems:"center",justifyContent:"center",
                fontFamily:"'Barlow Condensed',sans-serif",fontWeight:800,fontSize:"0.95rem",color:"#f5f5f5",cursor:"pointer",flexShrink:0
              }} onClick={() => onTabChange("account")}>
                {localUser.name[0].toUpperCase()}
              </div>
            ) : (
              <button onClick={() => setAuthOpen(true)} style={{
                background:"#fff",color:"#7f1d1d",border:"none",borderRadius:20,
        ...
        <div style={{ position:"relative",marginBottom:10 }}>
          <span style={{ position:"absolute",left:12,top:"50%",transform:"translateY(-50%)",fontSize:"0.95rem",color:"#6b7280",pointerEvents:"none" }}>🔍</span>
          <input
            ...
            style={{
              ...
              border:"none",borderRadius:10,color:"#f5f5f5",
      ...
      <div>
        ...
        {search || activeCat !== "All" ? (
          ...
          <>
            ...
            <SectionBlock title="🔋 Batteries" subtitle="AgroSky VOLT Plus series & chargers" products={featuredBatteries}
              onProductClick={onProductClick} onAddCart={(p) => { if(!localUser){setAuthOpen(true);return;} addToCart(p); }} user={localUser}
        ...
        <div style={{ padding:"20px 14px",borderTop:"1px solid #e8edf5",marginTop:10,background:"#fff" }}>
          <div style={{ display:"flex",alignItems:"center",gap:10,marginBottom:10 }}>
            <img src={LOGO} alt="AgroSky" style={{ height:28 }} />
            <span style={{ fontFamily:"'Barlow Condensed',sans-serif",fontSize:"0.9rem",fontWeight:800,color:"#111827" }}>AgroSky Drone Aspirant</span>
          </div>
          ...
          <div style={{ fontSize:"0.72rem",color:"#9ca3af",marginTop:10,textAlign:"center" }}>
            © 2025 AgroSky Drone Aspirant. All rights reserved.
          </div>
        </div>
      </div>
...
// ─── SECTION BLOCK ────────────────────────────────────────────
function SectionBlock({ title, subtitle, products, onProductClick, onAddCart, user, onViewAll, accent="#dc2626" }) {
  ...
  return (
    <div style={{ margin:"16px 0 0",background:"#ffffff",borderRadius:16,padding:"14px 0 4px",boxShadow:"0 1px 6px rgba(0,0,0,0.06)",marginLeft:14,marginRight:14 }}>
      <div style={{ display:"flex",alignItems:"center",justifyContent:"space-between",padding:"0 14px",marginBottom:12 }}>
        ...
        <button onClick={onViewAll} style={{ background:"#fff1f1",border:"1px solid #fca5a5",color:"#dc2626",borderRadius:20,padding:"5px 12px",fontFamily:"'DM Sans',sans-serif",fontWeight:700,fontSize:"0.75rem",cursor:"pointer" }}>
          View All →
        </button>
      </div>
    ...
    return (
      <div onClick={onClick} style={{
        ...
        <div style={{ padding:"8px 9px 8px",display:"flex",flexDirection:"column",gap:3 }}>
          <div style={{ fontSize:"0.56rem",fontWeight:700,color:"#fca5a5",letterSpacing:".05em",textTransform:"uppercase" }}>{p.category||"Product"}</div>
          ...
          <div style={{ display:"flex",alignItems:"center",gap:4,flexWrap:"wrap",marginTop:2 }}>
            <span style={{ fontFamily:"'Barlow Condensed',sans-serif",fontSize:"0.92rem",fontWeight:800,color:"#7f1d1d" }}>{formatINR(p.price)}</span>
            {off && <span style={{ fontSize:"0.58rem",background:"#fff1f1",border:"1px solid #fecaca",color:"#dc2626",padding:"1px 4px",borderRadius:8,fontWeight:700 }}>{off}%</span>}
          </div>
  ...
  return (
    <div onClick={onClick} style={{
      ...
      <div style={{ padding:"9px 10px 0",display:"flex",flexDirection:"column",flex:1,minHeight:0 }}>
        {/* Category */}
        <div style={{ fontSize:"0.58rem",fontWeight:700,color:"#fca5a5",letterSpacing:".07em",textTransform:"uppercase",marginBottom:3,lineHeight:1 }}>{p.category||"Product"}</div>
        {/* Name — 2 lines max */}
        ...
        <div style={{ display:"flex",alignItems:"center",gap:5,flexWrap:"wrap" }}>
          <span style={{ fontFamily:"'Barlow Condensed',sans-serif",fontSize:"1.05rem",fontWeight:800,color:"#7f1d1d" }}>{formatINR(p.price)}</span>
          {off && <span style={{ fontSize:"0.6rem",background:"#fff1f1",border:"1px solid #fecaca",color:"#dc2626",padding:"1px 5px",borderRadius:10,fontWeight:700 }}>{off}%</span>}
        </div>
        {/* Original price */}
        {p.originalPrice && (
      ...
      <div style={{ padding:"7px 10px 10px",display:"flex",gap:7,marginTop:"auto",flexShrink:0 }} onClick={e=>e.stopPropagation()}>
        <button onClick={onAddCart} style={{
          flex:1,background:"#7f1d1d",color:"#fff",border:"none",borderRadius:30,
        ...
        <a href={waLink(p.waNum||"919390238537",`Hello AgroSky! I'm interested in ${p.name} (${formatINR(p.price)})`)}
...
function CategoriesPage({ products: propProducts, onProductClick, onAddCart, user }) {
  ...
  return (
    <div style={{ background:"#f5f7fa",minHeight:"100vh",paddingBottom:70,fontFamily:"'DM Sans',sans-serif" }}>
      ...
      {/* ── Header ── */}
      <div style={{ background:"linear-gradient(135deg,#7f1d1d 0%,#dc2626 100%)",padding:"16px 16px 0" }}>
        <div style={{ fontFamily:"'Barlow Condensed',sans-serif",fontSize:"1.5rem",fontWeight:800,color:"#111827" }}>Categories</div>
                  ...
                  <div style={{ display:"flex",alignItems:"center",gap:12,marginBottom:12,
                    ...
                    <div style={{ flex:1,minWidth:0 }}>
                      ...
                      <div style={{ fontSize:"0.72rem",color:"#6b7280",marginTop:1 }}>
                        {descFor(name) || name}
                        {" · "}
                        <span style={{ fontWeight:600,color:"#dc2626" }}>{items.length} item{items.length!==1?"s":""}</span>
                      </div>
                    </div>
                  </div>
                  <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:10 }}>
...
function AccountPage({ user, onLogin, onLogout, cart, showToast }) {
  ...
  const statusColor = s => s==="enquired" ? "#dc2626" : s==="confirmed" ? "#dc2626" : "#f0a030";

  return (
    <div style={{ background:"#f5f7fa",minHeight:"100vh",paddingBottom:80,fontFamily:"'DM Sans',sans-serif" }}>
      <div style={{ background:"linear-gradient(135deg,#7f1d1d 0%,#dc2626 100%)",padding:"16px 16px 20px" }}>
        ...
        {user ? (
          <div style={{ display:"flex",alignItems:"center",gap:12,marginTop:10 }}>
            <div style={{ width:52,height:52,borderRadius:"50%",background:"#dc2626",display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"'Barlow Condensed',sans-serif",fontWeight:800,fontSize:"1.4rem",color:"#f5f5f5",flexShrink:0 }}>
      ...
      <div style={{ padding:"14px 14px" }}>
        {!user ? (
          <div style={{ textAlign:"center",paddingTop:24 }}>
            ...
            <div style={{ fontFamily:"'Barlow Condensed',sans-serif",fontSize:"1.4rem",fontWeight:800,color:"#111827",marginBottom:6 }}>Not signed in</div>
            <div style={{ fontSize:"0.85rem",color:"#6b7280",marginBottom:22 }}>Sign in to track orders and manage your profile</div>
            <button onClick={() => setAuthOpen(true)} style={{
              background:"#dc2626",color:"#f5f5f5",border:"none",borderRadius:40,padding:"13px 36px",
          ...
          <>
            <div style={{ display:"flex",gap:8,marginBottom:16 }}>
              {[["overview","🏠 Overview"],["orders","📦 My Orders"],["profile","👤 Profile"]].map(([s,l]) => (
                <button key={s} onClick={() => setActiveSection(s)} style={{
                  ...
                  background: activeSection===s ? "#dc2626" : "#ffffff",
                  color: activeSection===s ? "#f5f5f5" : "#6b7280",
                  border: activeSection===s ? "none" : "1px solid rgba(220,38,38,0.1)",
                  transition:"all .2s"
                }}>{l}</button>
              ))}
            </div>
              ...
              <>
                <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:10,marginBottom:14 }}>
                  {[
                    ...
                    <div key={label} style={{ background:"#ffffff",border:"1px solid rgba(220,38,38,0.1)",borderRadius:12,padding:"12px 10px",textAlign:"center" }}>
                      <div style={{ fontSize:"1.3rem",marginBottom:4 }}>{icon}</div>
                      <div style={{ fontFamily:"'Barlow Condensed',sans-serif",fontSize:"1.5rem",fontWeight:800,color:"#111827" }}>{val}</div>
                      <div style={{ fontSize:"0.68rem",color:"#6b7280",marginTop:1 }}>{label}</div>
                    </div>
                  ))}
                </div>

                {cartCount > 0 && (
                  <div style={{ background:"linear-gradient(135deg,rgba(220,38,38,0.1),rgba(220,38,38,0.02))",border:"1px solid rgba(220,38,38,0.2)",borderRadius:12,padding:"14px 16px",marginBottom:12 }}>
                    <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center" }}>
                      <div>
                        <div style={{ fontWeight:700,fontSize:"0.9rem",color:"#111827",marginBottom:2 }}>🛒 Active Cart</div>
                        <div style={{ fontSize:"0.78rem",color:"#6b7280" }}>{cartCount} item{cartCount!==1?"s":""} · {formatINR(cartTotal)}</div>
                      </div>
                      <a href={`https://wa.me/919390238537?text=${encodeURIComponent(`Hello AgroSky! I have ${cartCount} items in my cart worth ${formatINR(cartTotal)}.`)}`}
                        target="_blank" rel="noreferrer"
                        style={{ background:"#dc2626",color:"#f5f5f5",border:"none",borderRadius:20,padding:"7px 14px",fontFamily:"'DM Sans',sans-serif",fontWeight:700,fontSize:"0.78rem",cursor:"pointer",textDecoration:"none",flexShrink:0 }}>
                        Enquire 💬
                      </a>
                    </div>
                  </div>
                ...
                {[
                  ...
                  <div key={label} onClick={action||undefined} style={{ display:"flex",alignItems:"center",gap:12,padding:"13px 14px",background:"#ffffff",border:"1px solid rgba(220,38,38,0.1)",borderRadius:12,marginBottom:9,cursor:action?"pointer":"default" }}>
                    <span style={{ fontSize:"1.25rem" }}>{icon}</span>
                    <div style={{ flex:1 }}>
                      <div style={{ fontWeight:600,fontSize:"0.87rem",color:"#111827" }}>{label}</div>
                      <div style={{ fontSize:"0.73rem",color:"#6b7280",marginTop:1 }}>{sub}</div>
                    </div>
                    {action && <span style={{ color:"#6b7280",fontSize:"0.85rem" }}>›</span>}
                  </div>
                ))}
            ...
            {activeSection === "orders" && (
              <div>
                <div style={{ fontSize:"0.8rem",color:"#6b7280",marginBottom:12 }}>{enquiries.length} enquir{enquiries.length!==1?"ies":"y"} recorded</div>
                {enquiries.length === 0 ? (
                  <div style={{ textAlign:"center",padding:"40px 20px",color:"#6b7280" }}>
                    <div style={{ fontSize:"3rem",opacity:.3,marginBottom:10 }}>📦</div>
                    <div style={{ fontFamily:"'Barlow Condensed',sans-serif",fontSize:"1.2rem",fontWeight:800,color:"#111827",marginBottom:6 }}>No enquiries yet</div>
                    <div style={{ fontSize:"0.82rem" }}>When you enquire on WhatsApp, it'll appear here.</div>
                  </div>
                ) : enquiries.map(enq => (
                  <div key={enq.id} style={{ background:"#ffffff",border:"1px solid rgba(220,38,38,0.1)",borderRadius:12,padding:"14px 16px",marginBottom:10 }}>
                    <div style={{ display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:8 }}>
                      <div>
                        <div style={{ fontSize:"0.72rem",color:"#6b7280" }}>#{enq.id} · {new Date(enq.created_at).toLocaleDateString("en-IN",{day:"numeric",month:"short",year:"numeric"})}</div>
                        <div style={{ fontFamily:"'Barlow Condensed',sans-serif",fontSize:"1.1rem",fontWeight:800,color:"#111827",marginTop:2 }}>{formatINR(enq.total_amount)}</div>
                      </div>
                    ...
                    {Array.isArray(enq.items) && enq.items.map((item,i) => (
                      <div key={i} style={{ display:"flex",justifyContent:"space-between",fontSize:"0.78rem",color:"#6b7280",padding:"3px 0",borderTop: i===0?"1px solid rgba(220,38,38,0.08)":"none",marginTop: i===0?8:0 }}>
                        <span>{item.name} × {item.qty}</span>
                        <span style={{ color:"#111827" }}>{formatINR(item.price*item.qty)}</span>
                      </div>
                    ))}
                  </div>
                ))}
            ...
            {activeSection === "profile" && (
              <div>
                <div style={{ background:"#ffffff",border:"1px solid rgba(220,38,38,0.1)",borderRadius:12,padding:"16px",marginBottom:12 }}>
                  <div style={{ fontSize:"0.88rem",fontWeight:700,color:"#374151",marginBottom:14 }}>Personal Information</div>
                  {[["Full Name", user.name],["Email", user.email]].map(([label,val]) => (
                    <div key={label} style={{ marginBottom:12 }}>
                      <div style={{ fontSize:"0.7rem",color:"#6b7280",fontWeight:700,textTransform:"uppercase",letterSpacing:".07em",marginBottom:4 }}>{label}</div>
                      <div style={{ padding:"10px 13px",background:"rgba(0,0,0,0.04)",borderRadius:10,fontSize:"0.88rem",color:"rgba(0,0,0,0.5)",border:"1px solid rgba(220,38,38,0.08)" }}>{val}</div>
                    </div>
                  ))}
                  <div style={{ marginBottom:12 }}>
                    <div style={{ fontSize:"0.7rem",color:"#6b7280",fontWeight:700,textTransform:"uppercase",letterSpacing:".07em",marginBottom:4 }}>Phone Number</div>
                    <input value={profileForm.phone} onChange={e => setProfileForm(f=>({...f,phone:e.target.value}))} placeholder="+91 98765 43210"
                      style={{ width:"100%",padding:"10px 13px",background:"#ffffff",border:"1.5px solid rgba(220,38,38,0.2)",borderRadius:10,color:"#111827",fontFamily:"'DM Sans',sans-serif",fontSize:"0.88rem",outline:"none",boxSizing:"border-box" }} />
                  </div>
                  <div style={{ marginBottom:16 }}>
                    <div style={{ fontSize:"0.7rem",color:"#6b7280",fontWeight:700,textTransform:"uppercase",letterSpacing:".07em",marginBottom:4 }}>Delivery Address</div>
                    <textarea value={profileForm.address} onChange={e => setProfileForm(f=>({...f,address:e.target.value}))} placeholder="House No, Street, City, State, Pincode"
                      rows={3} style={{ width:"100%",padding:"10px 13px",background:"#ffffff",border:"1.5px solid rgba(220,38,38,0.2)",borderRadius:10,color:"#111827",fontFamily:"'DM Sans',sans-serif",fontSize:"0.88rem",outline:"none",boxSizing:"border-box",resize:"vertical" }} />
                  </div>
                  <button onClick={saveProfile} disabled={savingProfile} style={{
                    width:"100%",padding:"12px",background:"#dc2626",color:"#f5f5f5",border:"none",borderRadius:40,
...
function CartPage({ cart, user, showAuth, showToast, updateCartQty, removeFromCart, clearCart }) {
  ...
  return (
    <div style={{ background:"#f5f7fa",minHeight:"100vh",paddingBottom:100,fontFamily:"'DM Sans',sans-serif" }}>
      <div style={{ background:"linear-gradient(135deg,#7f1d1d 0%,#dc2626 100%)",padding:"16px 16px 14px" }}>
        <div style={{ fontFamily:"'Barlow Condensed',sans-serif",fontSize:"1.5rem",fontWeight:800,color:"#111827" }}>My Cart</div>
          ...
          <>
            {cart.map(item => (
              <div key={item.id} style={{ display:"flex",gap:12,padding:"12px",background:"#fff",border:"1.5px solid #e8edf5",borderRadius:14,marginBottom:10,boxShadow:"0 1px 4px rgba(0,0,0,0.05)" }}>
                ...
                <div style={{ flex:1 }}>
                  <div style={{ fontSize:"0.84rem",fontWeight:600,color:"#111827",marginBottom:3,lineHeight:1.3 }}>{item.name}</div>
                  <div style={{ fontFamily:"'Barlow Condensed',sans-serif",fontSize:"1rem",fontWeight:800,color:"#dc2626",marginBottom:8 }}>{formatINR(item.price*item.qty)}</div>
                  <div style={{ display:"flex",alignItems:"center",gap:8 }}>
                    <button onClick={() => updateCartQty(item.id, item.qty - 1)} style={{ width:28,height:28,borderRadius:"50%",border:"1.5px solid #fca5a5",background:"#fff1f1",color:"#dc2626",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",fontSize:"1rem" }}>−</button>
            ...
            <div style={{ background:"#fff",border:"1.5px solid #e8edf5",borderRadius:14,padding:"14px 16px",marginTop:8 }}>
              ...
              <div style={{ display:"flex",justifyContent:"space-between",fontSize:"0.85rem",color:"#6b7280",marginBottom:12 }}>
                <span>Delivery</span><span style={{ color:"#dc2626",fontWeight:600 }}>Contact for quote</span>
              </div>
              ...
              <button onClick={checkout} style={{
                width:"100%",padding:"13px",background:"#dc2626",color:"#fff",border:"none",borderRadius:40,
    ...
    <div style={{
      ...
      {tabs.map(tab => (
        <button key={tab.id} onClick={() => onTabChange(tab.id)} style={{
          ...
          {activeTab===tab.id && <div style={{ position:"absolute",bottom:0,width:28,height:2.5,background:"#dc2626",borderRadius:1 }} />}
        </button>
      ))}
    </div>
  );
...
const adminCSS = `
  ...
  .admin-sidebar{width:240px;background:#f3f4f6;border-right:1px solid #e5e7eb;display:flex;flex-direction:column;position:fixed;top:0;bottom:0;left:0;z-index:50;}
  .admin-sidebar-logo{padding:20px 18px;border-bottom:1px solid #e5e7eb;display:flex;align-items:center;gap:10px;}
  .admin-sidebar-logo img{height:34px;}
  .admin-sidebar-logo span{font-family:'Barlow Condensed',sans-serif;font-size:0.9rem;font-weight:800;color:#111827;line-height:1.2;}
  .admin-nav{flex:1;padding:16px 10px;display:flex;flex-direction:column;gap:2px;overflow-y:auto;}
  .admin-nav-label{font-size:0.63rem;font-weight:800;color:#9ca3af;letter-spacing:.12em;text-transform:uppercase;padding:0 10px;margin-bottom:6px;margin-top:10px;display:block;}
  .admin-link{display:flex;align-items:center;gap:10px;padding:10px 12px;border-radius:10px;cursor:pointer;background:none;border:none;font-family:'DM Sans',sans-serif;font-size:0.85rem;font-weight:500;color:#374151;transition:all .2s;text-align:left;width:100%;}
  .admin-link:hover{background:#fee2e2;color:#dc2626;}
  .admin-link.active{background:rgba(220,38,38,0.12);color:#dc2626;font-weight:700;}
  .admin-link-icon{font-size:1rem;width:20px;text-align:center;}
  .admin-link-badge{margin-left:auto;background:#fee2e2;border-radius:20px;padding:1px 8px;font-size:0.68rem;font-weight:700;color:#dc2626;}
  .admin-sidebar-footer{padding:14px 12px;border-top:1px solid #e5e7eb;}
  .admin-user{display:flex;align-items:center;gap:10px;}
  .admin-avatar{width:32px;height:32px;border-radius:50%;background:#dc2626;display:flex;align-items:center;justify-content:center;font-family:'Barlow Condensed',sans-serif;font-weight:800;color:#ffffff;font-size:0.9rem;flex-shrink:0;}
  .admin-user-name{font-size:0.82rem;font-weight:600;color:#fff;}
  .admin-user-role{font-size:0.72rem;color:#6b7280;}
  .logout-btn{margin-left:auto;background:none;border:none;color:#6b7280;cursor:pointer;font-size:1rem;transition:color .2s;}
  .logout-btn:hover{color:#e05050;}
  .admin-main{margin-left:240px;flex:1;background:#f9fafb;}
  .admin-topbar{display:flex;align-items:center;justify-content:space-between;padding:16px 28px;border-bottom:1px solid #e5e7eb;background:#f3f4f6;position:sticky;top:0;z-index:10;}
  ...
  .live-badge{display:flex;align-items:center;gap:6px;font-size:0.75rem;font-weight:700;color:#dc2626;background:rgba(220,38,38,0.1);border:1px solid rgba(220,38,38,0.3);border-radius:20px;padding:4px 10px;}
  .live-dot{width:6px;height:6px;border-radius:50%;background:#dc2626;animation:pulse 1.5s infinite;}
  ...
  .dash-stat{background:#ffffff;border:1px solid #e5e7eb;border-radius:16px;padding:18px 16px;transition:all .2s;cursor:pointer;}
  .dash-stat:hover{border-color:#dc2626;transform:translateY(-2px);}
  .dash-stat-icon{font-size:1.4rem;margin-bottom:8px;}
  .dash-stat-num{font-family:'Barlow Condensed',sans-serif;font-size:2.2rem;font-weight:800;color:#111827;line-height:1;}
  .dash-stat-label{font-size:0.78rem;color:#6b7280;margin-top:6px;}
  .dash-stat-trend{font-size:0.72rem;margin-top:4px;color:#dc2626;}
  .apps-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(260px,1fr));gap:16px;}
  .app-card{background:#ffffff;border:1px solid #e5e7eb;border-radius:13px;overflow:hidden;transition:all .2s;}
  .app-card:hover{border-color:rgba(220,38,38,0.6);}
  .app-card-visual{height:140px;position:relative;overflow:hidden;display:flex;align-items:center;justify-content:center;background:#f3f4f6;}
  ...
  .badge-active{background:rgba(220,38,38,0.12);border:1px solid #dc2626;color:#dc2626;}
  ...
  .badge-new{position:absolute;bottom:8px;left:8px;background:#dc2626;border:1px solid #dc2626;color:#ffffff;border-radius:20px;padding:2px 7px;font-size:0.63rem;font-weight:700;}
  ...
  .app-card-cat{font-size:0.73rem;color:#6b7280;margin-bottom:8px;}
  ...
  .admin-btn.green{background:#dc2626;color:#ffffff;}
  ...
  .admin-btn.blue{background:rgba(220,38,38,0.12);border:1px solid #dc2626;color:#dc2626;}
  .admin-btn.blue:hover{background:rgba(220,38,38,0.25);}
  ...
  .modal-box{background:#ffffff;border:1px solid rgba(220,38,38,0.25);border-radius:18px;padding:26px;width:100%;max-width:500px;position:relative;max-height:90vh;overflow-y:auto;}
  .modal-box::-webkit-scrollbar{width:4px;}.modal-box::-webkit-scrollbar-track{background:transparent;}.modal-box::-webkit-scrollbar-thumb{background:rgba(220,38,38,0.3);border-radius:2px;}
  .modal-title{font-family:'Barlow Condensed',sans-serif;font-size:1.35rem;font-weight:800;color:#fff;margin-bottom:20px;}
  .modal-field{margin-bottom:13px;}
  .modal-field label{font-size:0.7rem;color:#6b7280;font-weight:700;letter-spacing:.08em;text-transform:uppercase;display:block;margin-bottom:5px;}
  .modal-field input,.modal-field textarea,.modal-field select{width:100%;padding:10px 13px;background:#f9fafb;border:1.5px solid rgba(220,38,38,0.2);border-radius:10px;color:#111827;font-family:'DM Sans',sans-serif;font-size:0.88rem;outline:none;box-sizing:border-box;transition:border-color .2s;}
  .modal-field input:focus,.modal-field textarea:focus,.modal-field select:focus{border-color:#dc2626;}
  ...
  .admin-table th{text-align:left;padding:10px 14px;font-size:0.68rem;font-weight:800;color:#6b7280;letter-spacing:.1em;text-transform:uppercase;border-bottom:1px solid #e5e7eb;}
  .admin-table td{padding:12px 14px;border-bottom:1px solid #e5e7eb;color:#374151;vertical-align:middle;}
  .admin-table tr:hover td{background:rgba(220,38,38,0.04);}
  .admin-table tr:last-child td{border-bottom:none;}
  .user-avatar-sm{width:32px;height:32px;border-radius:50%;background:linear-gradient(135deg,#7f1d1d,#dc2626);display:flex;align-items:center;justify-content:center;font-family:'Barlow Condensed',sans-serif;font-weight:800;color:#fff;font-size:0.82rem;flex-shrink:0;}
  ...
  .search-bar{display:flex;align-items:center;gap:10px;background:#f9fafb;border:1px solid rgba(220,38,38,0.15);border-radius:12px;padding:9px 14px;margin-bottom:18px;}
  ...
  .chip{padding:5px 13px;border-radius:20px;font-size:0.78rem;font-weight:600;cursor:pointer;border:1px solid #e5e7eb;background:transparent;color:#6b7280;font-family:'DM Sans',sans-serif;transition:all .2s;}
  .chip.active{background:rgba(220,38,38,0.15);border-color:#dc2626;color:#dc2626;}
  ...
  .card-table{background:#ffffff;border:1px solid #e5e7eb;border-radius:14px;overflow:hidden;}
  .gradient-preview{height:32px;border-radius:6px;margin-top:6px;}
  .tab-pills{display:flex;background:#f3f4f6;border-radius:40px;padding:3px;margin-bottom:20px;width:fit-content;}
  .tab-pill{padding:7px 18px;border-radius:40px;border:none;cursor:pointer;font-family:'DM Sans',sans-serif;font-size:0.82rem;font-weight:600;transition:all .2s;color:#6b7280;background:transparent;}
  .tab-pill.active{background:rgba(220,38,38,0.15);color:#dc2626;}
`;
...
function AdminBanners({ showToast }) {
  ...
  const [editIdx, setEditIdx] = useState(null);
  const [loading, setLoading] = useState(false);
  const emptyForm = { title:"", subtitle:"", badge:"", bg:"linear-gradient(135deg,#7f1d1d 0%,#dc2626 60%,#7f1d1d 100%)", emoji:"🚁", cta:"Shop Now", active:true };
  ...
  if (editIdx !== null) return (
    <div>
      <button onClick={()=>setEditIdx(null)} style={{ display:"flex",alignItems:"center",gap:6,background:"none",border:"none",color:"#6b7280",cursor:"pointer",fontFamily:"'DM Sans',sans-serif",fontSize:"0.85rem",marginBottom:20,padding:0 }}>← Back to Banners</button>
      <div style={{ background:"#ffffff",border:"1px solid rgba(220,38,38,0.2)",borderRadius:16,padding:24,maxWidth:560 }}>
        ...
        <div className="modal-field">
          ...
          <input value={form.bg} onChange={e=>setForm(f=>({...f,bg:e.target.value}))} placeholder="linear-gradient(135deg,#7f1d1d 0%,...)" style={{ fontFamily:"monospace",fontSize:"0.8rem" }} />
        ...
        <div style={{ display:"flex",gap:10,marginTop:20 }}>
          ...
          <button className="admin-btn" onClick={()=>setEditIdx(null)} style={{ flex:1,justifyContent:"center",background:"rgba(255,255,255,0.05)",color:"#1f2937",border:"1px solid rgba(255,255,255,0.1)" }}>Cancel</button>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div>
      ...
      <div style={{ display:"flex",flexDirection:"column",gap:12 }}>
        {banners.map((b,i) => (
          <div key={b.id||i} style={{ background:"#ffffff",border:"1px solid #e5e7eb",borderRadius:14,padding:"14px 18px",display:"flex",alignItems:"center",gap:16,flexWrap:"wrap" }}>
            ...
            <div style={{ flex:1,minWidth:120 }}>
              <div style={{ fontSize:"0.8rem",color:"#6b7280",marginBottom:4 }}>Badge: <span style={{ color:"#111827" }}>{b.badge}</span></div>
              <div style={{ fontSize:"0.8rem",color:"#6b7280" }}>CTA: <span style={{ color:"#dc2626" }}>{b.cta}</span></div>
            </div>
            <div style={{ display:"flex",alignItems:"center",gap:8 }}>
              <label style={{ display:"flex",alignItems:"center",gap:6,cursor:"pointer",fontSize:"0.78rem",color:b.active?"#dc2626":"#6b7280" }}>
                <input type="checkbox" checked={!!b.active} onChange={()=>toggleActive(i)} style={{ accentColor:"#dc2626" }} /> {b.active?"Live":"Hidden"}
              </label>
  ...
  return (
    <div>
      <div className="section-hdr">
        ...
        <span style={{ fontSize:"0.83rem",color:"#6b7280" }}>{offers.length} active offer{offers.length!==1?"s":""}</span>
      </div>
      <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:20 }}>
        <div>
          <div style={{ fontFamily:"'Barlow Condensed',sans-serif",fontSize:"1rem",fontWeight:800,color:"#dc2626",marginBottom:12 }}>🏷️ On Offer ({offers.length})</div>
          <div style={{ display:"flex",flexDirection:"column",gap:10 }}>
            {offers.map(p=>(
              <div key={p.id} style={{ background:"#ffffff",border:"1px solid rgba(255,149,0,0.25)",borderRadius:12,padding:"12px 14px",display:"flex",alignItems:"center",gap:12 }}>
                <div style={{ width:44,height:44,borderRadius:8,overflow:"hidden",background:"#f0f0f0",flexShrink:0 }}>{p.image&&<img src={p.image} alt="" style={{ width:"100%",height:"100%",objectFit:"cover" }} />}</div>
                <div style={{ flex:1,minWidth:0 }}>
                  <div style={{ fontSize:"0.83rem",fontWeight:700,color:"#111827",lineHeight:1.3,marginBottom:2,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap" }}>{p.name}</div>
                  <div style={{ fontSize:"0.78rem",color:"#dc2626",fontFamily:"'Barlow Condensed',sans-serif",fontWeight:700 }}>{formatINR(p.price)} {p.originalPrice&&<span style={{ color:"#6b7280",textDecoration:"line-through",fontWeight:400,fontSize:"0.75rem" }}>{formatINR(p.originalPrice)}</span>}</div>
            ...
            {offers.length===0 && <div style={{ color:"#6b7280",fontSize:"0.83rem",padding:14,textAlign:"center" }}>No offers active</div>}
          </div>
        </div>
        <div>
          <div style={{ fontFamily:"'Barlow Condensed',sans-serif",fontSize:"1rem",fontWeight:800,color:"#6b7280",marginBottom:12 }}>📦 Not on Offer ({nonOffers.length})</div>
          <div style={{ display:"flex",flexDirection:"column",gap:10 }}>
            {nonOffers.map(p=>(
              <div key={p.id} style={{ background:"#ffffff",border:"1px solid #e5e7eb",borderRadius:12,padding:"12px 14px",display:"flex",alignItems:"center",gap:12 }}>
                <div style={{ width:44,height:44,borderRadius:8,overflow:"hidden",background:"#f0f0f0",flexShrink:0 }}>{p.image&&<img src={p.image} alt="" style={{ width:"100%",height:"100%",objectFit:"cover" }} />}</div>
                <div style={{ flex:1,minWidth:0 }}>
                  <div style={{ fontSize:"0.83rem",fontWeight:700,color:"#111827",lineHeight:1.3,marginBottom:2,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap" }}>{p.name}</div>
                  <div style={{ fontSize:"0.78rem",color:"#dc2626",fontFamily:"'Barlow Condensed',sans-serif",fontWeight:700 }}>{formatINR(p.price)}</div>
                </div>
  ...
  return (
    <div>
      <div className="section-hdr">
        ...
        <span style={{ fontSize:"0.83rem",color:"#6b7280" }}>{users.length} registered user{users.length!==1?"s":""}</span>
      </div>
      ...
      <div className="search-bar">
        ...
        {search && <button onClick={()=>setSearch("")} style={{ background:"none",border:"none",color:"#6b7280",cursor:"pointer",fontSize:"1rem" }}>✕</button>}
      </div>
      {loading ? (
          ...
          <table className="admin-table">
            ...
            <tbody>
              {filteredUsers.map(u => {
                ...
                return (
                  <tr key={u.id}>
                    <td><div style={{ display:"flex",alignItems:"center",gap:10 }}><div className="user-avatar-sm">{initials}</div><div style={{ fontWeight:600,color:"#111827",fontSize:"0.85rem" }}>{u.full_name||"—"}</div></div></td>
                    <td style={{ color:"#6b7280",fontSize:"0.82rem" }}>{u.email||"—"}</td>
                    <td style={{ color:"#6b7280",fontSize:"0.82rem" }}>{u.phone||"—"}</td>
                    <td style={{ color:"#6b7280",fontSize:"0.78rem" }}>{u.created_at?new Date(u.created_at).toLocaleDateString("en-IN",{day:"numeric",month:"short",year:"numeric"}):"—"}</td>
                    <td><span style={{ fontFamily:"'Barlow Condensed',sans-serif",fontWeight:800,color:userEnqs>0?"#dc2626":"#6b7280",fontSize:"1rem" }}>{userEnqs}</span></td>
                  </tr>
                );
              })}
          ...
          <table className="admin-table">
            ...
            <tbody>
              {filteredEnqs.map((e,i) => {
                ...
                return (
                  <tr key={e.id||i}>
                    <td><div style={{ display:"flex",alignItems:"center",gap:10 }}><div className="user-avatar-sm" style={{ background:"linear-gradient(135deg,#7f1d1d,#dc2626)" }}>{initials}</div><div style={{ fontWeight:600,color:"#fff",fontSize:"0.85rem" }}>{name}</div></div></td>
                    <td style={{ maxWidth:280 }}>
                      {items && Array.isArray(items) ? (
                        <div style={{ fontSize:"0.78rem",color:"#6b7280" }}>{items.map(it=>`${it.name} ×${it.qty}`).join(", ")}</div>
                      ) : (
                        <div style={{ fontSize:"0.78rem",color:"#6b7280",whiteSpace:"pre-wrap" }}>{e.message||"—"}</div>
                      )}
                    </td>
                    <td style={{ fontFamily:"'Barlow Condensed',sans-serif",fontWeight:800,color:"#dc2626",fontSize:"1rem" }}>{e.total ? formatINR(e.total) : "—"}</td>
                    <td style={{ color:"#6b7280",fontSize:"0.78rem" }}>{e.created_at?new Date(e.created_at).toLocaleDateString("en-IN",{day:"numeric",month:"short",year:"numeric"}):"—"}</td>
                  </tr>
                );
              })}
  ...
  return (
    <div>
      ...
      <div className="filter-chips">
        ...
        <div style={{ width:1,background:"rgba(220,38,38,0.2)",margin:"0 4px" }} />
          ...
          {filtered.map(p => (
            <div key={p.id} className="app-card">
              ...
              <div className="app-card-body">
                ...
                <div style={{ fontFamily:"'Barlow Condensed',sans-serif",fontWeight:800,fontSize:"1.1rem",color:"#dc2626",marginBottom:4 }}>{formatINR(p.price)}</div>
                {p.originalPrice && <div style={{ fontSize:"0.75rem",color:"#6b7280",marginBottom:10 }}>MRP: <span style={{ textDecoration:"line-through" }}>{formatINR(p.originalPrice)}</span> <span style={{ color:"#ff9500" }}>({discount(p.price,p.originalPrice)}% off)</span></div>}
                <div className="app-card-actions">
                  <button className="admin-btn blue sm" onClick={()=>{setEditProduct(p);setShowModal(true);}}>✏️ Edit</button>
                  <button className="admin-btn sm" onClick={()=>toggleStatus(p)} style={{ background:"rgba(220,38,38,0.08)",border:"1px solid rgba(220,38,38,0.3)",color:"#dc2626" }}>{p.status==="outofstock"?"✅":"⛔"}</button>
  ...
  return (
    <div>
      <div style={{ marginBottom:28 }}>
        <div style={{ fontFamily:"'Barlow Condensed',sans-serif",fontSize:"1.6rem",fontWeight:800,color:"#111827",marginBottom:4 }}>Welcome back, Admin 👋</div>
        <div style={{ fontSize:"0.85rem",color:"#6b7280" }}>Here's a snapshot of AgroSky Drone Aspirant.</div>
      </div>
      ...
      <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:16 }}>
        <div style={{ background:"#ffffff",border:"1px solid #e5e7eb",borderRadius:14,padding:"18px 20px" }}>
          <div style={{ fontFamily:"'Barlow Condensed',sans-serif",fontSize:"1rem",fontWeight:800,color:"#111827",marginBottom:14 }}>📊 Inventory by Category</div>
            ...
            return (
              <div key={cat} style={{ marginBottom:12 }}>
                <div style={{ display:"flex",justifyContent:"space-between",fontSize:"0.8rem",color:"#6b7280",marginBottom:4 }}>
                  <span>{cat}</span><span style={{ color:"#111827",fontWeight:600 }}>{cnt}</span>
                </div>
                <div style={{ height:6,background:"rgba(220,38,38,0.1)",borderRadius:3,overflow:"hidden" }}>
                  <div style={{ height:"100%",width:`${pct}%`,background:"linear-gradient(90deg,#991b1b,#dc2626)",borderRadius:3,transition:"width .6s" }} />
                </div>
              </div>
            );
          })}
        ...
        <div style={{ background:"#ffffff",border:"1px solid #e5e7eb",borderRadius:14,padding:"18px 20px" }}>
          <div style={{ fontFamily:"'Barlow Condensed',sans-serif",fontSize:"1rem",fontWeight:800,color:"#111827",marginBottom:14 }}>⚡ Quick Actions</div>
...
function AdminCategories({ showToast }) {
  ...
  if (editIdx !== null) return (
    <div>
      <button onClick={()=>setEditIdx(null)} style={{ display:"flex",alignItems:"center",gap:6,background:"none",border:"none",color:"#6b7280",cursor:"pointer",fontFamily:"'DM Sans',sans-serif",fontSize:"0.85rem",marginBottom:20,padding:0 }}>← Back to Categories</button>
      <div style={{ background:"#ffffff",border:"1px solid rgba(220,38,38,0.2)",borderRadius:16,padding:24,maxWidth:520 }}>
          ...
          <div style={{ display:"flex",flexWrap:"wrap",gap:8,marginBottom:10 }}>
            {COMMON_ICONS.map(ic => (
              <button key={ic} onClick={()=>setForm(f=>({...f,icon:ic}))} style={{
                width:38,height:38,borderRadius:10,border: form.icon===ic?"2.5px solid #dc2626":"1px solid rgba(220,38,38,0.2)",
                background: form.icon===ic?"rgba(220,38,38,0.15)":"#f5f5f5",fontSize:"1.2rem",cursor:"pointer",
                display:"flex",alignItems:"center",justifyContent:"center",transition:"all .15s"
              }}>{ic}</button>
            ))}
          </div>
          <input value={form.icon} onChange={e=>setForm(f=>({...f,icon:e.target.value}))} placeholder="Or type any emoji"
            style={{ width:"100%",padding:"9px 12px",background:"#ffffff",border:"1.5px solid rgba(220,38,38,0.2)",borderRadius:10,color:"#111827",fontFamily:"'DM Sans',sans-serif",fontSize:"1.2rem",outline:"none",boxSizing:"border-box",textAlign:"center" }} />
        </div>
        ...
        {/* Preview */}
        <div style={{ background:"#f5f5f5",border:"1px solid rgba(220,38,38,0.15)",borderRadius:12,padding:"12px 16px",marginBottom:18 }}>
          <div style={{ fontSize:"0.7rem",color:"#6b7280",marginBottom:8,textTransform:"uppercase",letterSpacing:".07em",fontWeight:700 }}>Preview</div>
          <div style={{ display:"flex",alignItems:"center",gap:12 }}>
            <div style={{ width:48,height:48,borderRadius:14,background:"linear-gradient(135deg,#7f1d1d,#dc2626)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:"1.5rem" }}>{form.icon||"📦"}</div>
            <div>
              <div style={{ fontFamily:"'Barlow Condensed',sans-serif",fontWeight:800,fontSize:"1.1rem",color:"#111827" }}>{form.name||"Category Name"}</div>
              <div style={{ fontSize:"0.73rem",color:"#6b7280",marginTop:1 }}>{form.description||"No description"}</div>
            </div>
          </div>
        </div>

        <label style={{ display:"flex",alignItems:"center",gap:10,cursor:"pointer",marginBottom:20,color:"#6b7280",fontSize:"0.85rem",fontFamily:"'DM Sans',sans-serif" }}>
          <input type="checkbox" checked={!!form.active} onChange={e=>setForm(f=>({...f,active:e.target.checked}))} style={{ accentColor:"#dc2626",width:16,height:16 }} />
          Active (visible in app and product form)
        </label>

        <div style={{ display:"flex",gap:10 }}>
          <button onClick={saveForm} disabled={saving||!form.name.trim()} style={{ flex:1,padding:"11px",background:saving||!form.name.trim()?"rgba(220,38,38,0.4)":"#dc2626",color:"#f5f5f5",border:"none",borderRadius:40,fontFamily:"'DM Sans',sans-serif",fontWeight:700,cursor:saving?"wait":"pointer" }}>{saving?"Saving…":"Save Category"}</button>
          <button onClick={()=>setEditIdx(null)} style={{ flex:1,padding:"11px",background:"rgba(255,255,255,0.05)",color:"#1f2937",border:"1px solid rgba(255,255,255,0.1)",borderRadius:40,fontFamily:"'DM Sans',sans-serif",fontWeight:600,cursor:"pointer" }}>Cancel</button>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div>
      ...
      {/* Info banner */}
      <div style={{ background:"rgba(220,38,38,0.06)",border:"1px solid rgba(220,38,38,0.18)",borderRadius:12,padding:"10px 16px",marginBottom:20,fontSize:"0.8rem",color:"#6b7280",display:"flex",alignItems:"center",gap:8 }}>
        <span style={{ fontSize:"1.1rem" }}>💡</span>
        Categories created here appear in the <strong style={{ color:"#dc2626" }}>Add/Edit Product</strong> form and in the app's <strong style={{ color:"#dc2626" }}>Browse</strong> sections. Drag ↕ arrows to reorder.
      </div>

      {loading ? (
        <div style={{ textAlign:"center",padding:"40px 0",color:"#6b7280" }}>Loading categories…</div>
      ) : (
        <div style={{ display:"flex",flexDirection:"column",gap:10 }}>
          {cats.map((c,i) => (
            <div key={c.db_id||c.name||i} style={{
              background:"#ffffff",border:`1px solid ${c.active?"rgba(220,38,38,0.2)":"rgba(255,255,255,0.06)"}`,
              ...
              <div style={{ display:"flex",flexDirection:"column",gap:2,flexShrink:0 }}>
                <button onClick={()=>moveRow(i,-1)} disabled={i===0} style={{ background:"none",border:"none",color:i===0?"#2d3d2e":"#6b7280",cursor:i===0?"default":"pointer",fontSize:"0.75rem",lineHeight:1,padding:"1px 4px" }}>▲</button>
                <button onClick={()=>moveRow(i,+1)} disabled={i===cats.length-1} style={{ background:"none",border:"none",color:i===cats.length-1?"#2d3d2e":"#6b7280",cursor:i===cats.length-1?"default":"pointer",fontSize:"0.75rem",lineHeight:1,padding:"1px 4px" }}>▼</button>
              </div>

              {/* Icon */}
              <div style={{ width:46,height:46,borderRadius:12,background:"linear-gradient(135deg,#7f1d1d,#dc2626)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:"1.4rem",flexShrink:0 }}>{c.icon||"📦"}</div>

              {/* Info */}
              <div style={{ flex:1,minWidth:0 }}>
                <div style={{ display:"flex",alignItems:"center",gap:8 }}>
                  <div style={{ fontFamily:"'Barlow Condensed',sans-serif",fontSize:"1rem",fontWeight:800,color:"#111827" }}>{c.name}</div>
                ...
                <div style={{ fontSize:"0.74rem",color:"#6b7280",marginTop:2,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap" }}>{c.description||"No description"}</div>
              </div>

              {/* Actions */}
              <div style={{ display:"flex",alignItems:"center",gap:8,flexShrink:0 }}>
                <label style={{ display:"flex",alignItems:"center",gap:5,cursor:"pointer",fontSize:"0.75rem",color:c.active?"#dc2626":"#6b7280" }}>
                  <input type="checkbox" checked={!!c.active} onChange={()=>toggleActive(i)} style={{ accentColor:"#dc2626" }} />
                  {c.active?"Live":"Off"}
                </label>
                <button className="admin-btn blue sm" onClick={()=>openEdit(i)}>✏️ Edit</button>
...
function AdminPage({ autoAuthed = false }) {
  ...
  if (!authed) return (
    <div style={{ minHeight:"100vh",background:"#f5f5f5",display:"flex",alignItems:"center",justifyContent:"center",padding:20 }}>
      <div style={{ background:"#ffffff",border:"1px solid rgba(220,38,38,0.2)",borderRadius:20,padding:"36px 28px",width:"100%",maxWidth:380,textAlign:"center" }}>
        <img src={LOGO} alt="AgroSky" style={{ height:44,marginBottom:16,borderRadius:8 }} />
        <div style={{ fontFamily:"'Barlow Condensed',sans-serif",fontSize:"1.8rem",fontWeight:800,color:"#111827",marginBottom:6 }}>Admin Portal</div>
        <div style={{ fontSize:"0.82rem",color:"#6b7280",marginBottom:28 }}>AgroSky Drone Aspirant</div>
        <input type="password" placeholder="Enter admin password" value={pw} onChange={e=>{setPw(e.target.value);setErr("");}}
          ...
          style={{ width:"100%",padding:"13px 16px",background:"#f5f5f5",border:"1.5px solid rgba(220,38,38,0.2)",borderRadius:12,color:"#fff",fontFamily:"'DM Sans',sans-serif",fontSize:"0.92rem",outline:"none",boxSizing:"border-box",marginBottom:10,letterSpacing:4 }} />
        ...
        <button onClick={()=>pw==="sag@admin2025"?(sessionStorage.setItem("sag_admin","1"),setAuthed(true)):setErr("Incorrect password. Please try again.")}
          style={{ width:"100%",padding:"13px",background:"#dc2626",color:"#f5f5f5",border:"none",borderRadius:40,fontFamily:"'DM Sans',sans-serif",fontWeight:700,fontSize:"0.95rem",cursor:"pointer" }}>
          Enter Admin Portal →
        </button>
      </div>
    </div>
    ...
    <div className="admin-layout">
      ...
      <aside className="admin-sidebar">
        <div className="admin-sidebar-logo">
          <img src={LOGO} alt="AgroSky" />
          <span>AgroSky Admin<br /><span style={{ fontSize:"0.68rem",color:"#6b7280",fontFamily:"'DM Sans',sans-serif",fontWeight:400 }}>Control Panel</span></span>
        </div>
      ...
      <main className="admin-main">
        <div className="admin-topbar">
          ...
          <div className="admin-topbar-right">
            <div style={{ fontSize:"0.75rem",color:"#6b7280" }}>{new Date().toLocaleDateString("en-IN",{weekday:"short",day:"numeric",month:"short"})}</div>
      ...
      <div style={{ position:"fixed",bottom:24,right:24,zIndex:99999,background:toast.type==="success"?"#1a4a2a":"#4a1a1a",border:`1px solid ${toast.type==="success"?"#dc2626":"#e05050"}`,borderRadius:12,padding:"11px 20px",color:"#fff",fontSize:"0.85rem",fontFamily:"'DM Sans',sans-serif",transition:"all .35s",transform:toast.show?"translateY(0)":"translateY(20px)",opacity:toast.show?1:0,pointerEvents:toast.show?"all":"none",boxShadow:"0 10px 30px rgba(0,0,0,0.5)" }}>{toast.msg}</div>
    </div>
  );
}
  ...
  return (
    <div className="modal-overlay" onClick={e=>e.target===e.currentTarget&&onClose()}>
      <div className="modal-box">
        ...
        {[["Product Name","name","text","e.g. AgroSky Agri Drone 10L"],["Price (₹)","price","number","e.g. 750000"],["Original Price (₹)","originalPrice","number","e.g. 900000"],["Image URL","image","text","https://..."],["WhatsApp Number","waNum","text","919390238537"]].map(([label,key,type,ph]) => (
        ...
        <div className="modal-field">
          ...
          {newCatMode && (
            <div style={{ background:"rgba(220,38,38,0.06)",border:"1px solid rgba(220,38,38,0.2)",borderRadius:10,padding:"10px 12px",marginBottom:10 }}>
              <div style={{ fontSize:"0.72rem",color:"#6b7280",fontWeight:700,marginBottom:8,textTransform:"uppercase",letterSpacing:".06em" }}>Quick-add Category</div>
              <div style={{ display:"flex",gap:8,marginBottom:8 }}>
                <input value={newCatIcon} onChange={e=>setNewCatIcon(e.target.value)} placeholder="📦"
                  style={{ width:44,padding:"8px",background:"#f5f5f5",border:"1.5px solid rgba(220,38,38,0.2)",borderRadius:8,color:"#fff",fontFamily:"'DM Sans',sans-serif",fontSize:"1.1rem",textAlign:"center",outline:"none" }} />
                <input value={newCatName} onChange={e=>setNewCatName(e.target.value)}
                  placeholder="e.g. Spare Parts" onKeyDown={e=>e.key==="Enter"&&addQuickCategory()}
                  style={{ flex:1,padding:"8px 12px",background:"#f5f5f5",border:"1.5px solid rgba(220,38,38,0.2)",borderRadius:8,color:"#fff",fontFamily:"'DM Sans',sans-serif",fontSize:"0.88rem",outline:"none" }} />
              </div>
              <button onClick={addQuickCategory} disabled={addingCat||!newCatName.trim()} style={{
                width:"100%",padding:"8px",background:addingCat||!newCatName.trim()?"rgba(220,38,38,0.3)":"#dc2626",
                color:"#f5f5f5",border:"none",borderRadius:8,fontFamily:"'DM Sans',sans-serif",
          ...
          {catLoading ? (
            <div style={{ padding:"10px 12px",background:"#f5f5f5",border:"1.5px solid rgba(220,38,38,0.2)",borderRadius:10,color:"#6b7280",fontSize:"0.85rem" }}>Loading categories…</div>
          ) : (
            <select value={form.category} onChange={e=>set("category",e.target.value)}
              size={Math.min(categories.length, 6)}
              style={{ width:"100%",background:"#f5f5f5",border:"1.5px solid rgba(220,38,38,0.2)",
                borderRadius:10,color:"#111827",fontFamily:"'DM Sans',sans-serif",fontSize:"0.88rem",outline:"none",
                boxSizing:"border-box",minHeight:44,padding:0,cursor:"pointer",overflowY:"auto" }}>
              {categories.map(c => (
                <option key={c} value={c} style={{ padding:"8px 12px",background: form.category===c?"#dc2626":"#ffffff",color:"#fff",fontSize:"0.88rem" }}>{c}</option>
              ))}
            </select>
          )}
          <div style={{ fontSize:"0.72rem",color:"#6b7280",marginTop:5 }}>
            Selected: <strong style={{ color:"#dc2626" }}>{form.category || "—"}</strong>
        ...
        <div style={{ display:"flex",gap:16,marginBottom:18 }}>
          {[["isNew","New Arrival"],["isOffer","On Offer"]].map(([k,l]) => (
            <label key={k} style={{ display:"flex",alignItems:"center",gap:8,cursor:"pointer",color:"#6b7280",fontSize:"0.85rem" }}>
              <input type="checkbox" checked={form[k]||false} onChange={e=>set(k,e.target.checked)} style={{ accentColor:"#dc2626" }} />{l}
            </label>
          ))}
        </div>
        <div style={{ display:"flex",gap:10 }}>
          <button onClick={()=>onSave({...form,price:Number(form.price),originalPrice:Number(form.originalPrice)||null})} disabled={saving} style={{ flex:1,padding:"12px",background:"#dc2626",color:"#f5f5f5",border:"none",borderRadius:40,fontFamily:"'DM Sans',sans-serif",fontWeight:700,fontSize:"0.9rem",cursor:saving?"wait":"pointer",opacity:saving?0.7:1 }}>{saving?"Saving…":"Save Product"}</button>
          <button onClick={onClose} style={{ flex:1,padding:"12px",background:"rgba(255,255,255,0.05)",color:"#1f2937",border:"1px solid rgba(255,255,255,0.1)",borderRadius:40,fontFamily:"'DM Sans',sans-serif",fontWeight:600,cursor:"pointer" }}>Cancel</button>
        </div>
      </div>
    </div>
  );
...
export default function App() {
  ...
  useEffect(() => {
    ...
    // Register Capacitor listener if running inside Android WebView
    // window.Capacitor is injected by the native bridge — no npm package needed
    if (window.Capacitor && window.Capacitor.Plugins && window.Capacitor.Plugins.App) {
      ...
      }).then(listener => { capacitorListener = listener; }).catch(() => {});
    }
  ...
  if (showAdmin) return (
    <div>
      ...
      <div style={{ position:"fixed",bottom:20,right:20,zIndex:9999 }}>
        <button onClick={()=>setShowAdmin(false)} style={{ background:"#ffffff",border:"1px solid rgba(220,38,38,0.3)",color:"#6b7280",padding:"8px 14px",borderRadius:40,fontSize:"0.75rem",cursor:"pointer",fontFamily:"'DM Sans',sans-serif" }}>← Back to App</button>
      </div>
    </div>
  );
    ...
    <div style={{ fontFamily:"'DM Sans',sans-serif",background:"#f5f7fa",minHeight:"100vh" }}>
      ...
      {splash && (
        <div className={`splash-wrap${splashOut ? " out" : ""}`}>
          <img src={SAG_LOGO_B64} className="splash-logo-img" alt="AgroSky" />
        </div>
      )}
      ...
      {(user?.email||"").toLowerCase() === ADMIN_EMAIL && (
        <div style={{ position:"fixed",bottom:72,right:16,zIndex:99 }}>
          <button onClick={openAdmin} style={{ background:"rgba(10,15,13,0.9)",border:"1px solid rgba(220,38,38,0.2)",color:"#6b7280",padding:"7px 13px",borderRadius:40,fontSize:"0.72rem",cursor:"pointer",fontFamily:"'DM Sans',sans-serif",backdropFilter:"blur(6px)" }}>⚙ Admin</button>
        </div>
      )}
    </div>
  );
